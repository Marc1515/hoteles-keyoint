...
if(isset($_POST)) {
    // --- LLAMAR WS : InsertarLocker 
    $parametros_insertarLocker = array('token'=>$token_ws, 'IdLocker'=>$idlocker, 'NumeroSerie'=>$numserie, 'Nombre'=>$nombre, 'Estado'=>$estado, 'VPN'=>$vpn, 'BD'=>$bd,, 'UrlMaps'=>$urlMap);
    $soap_result_IL = $SoapClient_KeysConsigna->InsertarLocker($parametros_insertarLocker);

    if($soap_result_IL->InsertarLockerResult == TRUE){
